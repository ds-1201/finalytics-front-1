...
function SignIn() {
    ...
    const handleCredentialsSubmit=(e)=>{
        ...
        axios.post("https://finalyticsad.fintract.co.uk/auth/login/",`email=${credentials.email}&password=${credentials.password}`)
        .then(res => {
    ...
    const handleEmailVerification=e=>{
        e.preventDefault();
        setErrorMessages({});
        axios.post("https://finalyticsad.fintract.co.uk/auth/refresh-token/",`email=${credentials.email}`)
        .then(res => {
    ...
    const handlePasswordReset=e=>{
        ...
        window.alert("Email field cannot be empty")
            ...
            {
                axios.post("https://finalyticsad.fintract.co.uk/auth/reset-password/",`email=${credentials.email}`)
                .then(res => {
    ...
    const handleOtpSubmit=(e)=>{
        ...
        axios.post(`https://finalyticsad.fintract.co.uk/auth/login/${userId}/`,`otp=${otp}`)
            .then(res=>{
    ...
    return (
        <Auth>
                ...
                />
                {
                    errorMessages.password&&<SubLine>{errorMessages.password}</SubLine>
                }
                <button className="signin-button" type="submit"><p className="signin-button-text">Sign In</p></button>
                </form>
                {
                    (status==401&&
                    errorMessages.detail=="Email is not verified")&&
                    <div onClick={handleEmailVerification} className="email-verify-tag"
                    style={{color:"white",fontSize:"20px",textDecoration:"underline",marginTop:"10px",cursor:"pointer"}}>Please verify your email first
                    </div>
                    
                }
                {
                    status==401&&errorMessages.detail!="Email is not verified"&&
                    <SubLine style={{marginTop:"10px"}} size="20px">{errorMessages.detail}</SubLine>
                }
    
                {
                    status===1&&<div style={{color:"white",fontSize:"20px", marginTop:"10px"}}>Verification Link Sent!</div>
                }
                <div>
                    <p className="footer-text">Don't have an account?<a className="footer-text-link" href={"/signup"}> Sign Up</a></p>
                    <div onClick={handlePasswordReset}><p className="footer-text" style={{textDecoration:"underline",cursor:"pointer"}}>Forgot Password? </p></div>
                    {
                        resetPasswordStatus==200&&<p className="footer-text">{Object.values(resetPasswordMesssage)[0]}</p>
                    }
                    {
                        resetPasswordStatus==400&&<p>{Object.values(resetPasswordMesssage)[0]}</p>
                    }
                </div>
                </div>
                }
